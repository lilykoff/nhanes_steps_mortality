...
cor_mat = 
  df_pa %>% select(contains("mean")) %>%
  cor(., use = "complete", method = "pearson") 
pvals = df_pa %>% select(contains("mean")) %>%
  rstatix::cor_pmat(.) %>% 
  select(-rowname) %>% 
  as.matrix()
...
cor_mat = df_pa %>% 
  select(mean_acti_steps, mean_adept_steps,
         mean_oak_steps_80, mean_scrf_steps,
         mean_scssl_steps, mean_vs_steps_15,
         mean_vs_revised_steps_15, mean_AC, mean_log10AC,
         mean_PAXMTSM, mean_log10PAXMTSM) %>% 
  cor(., use = "complete", method = "pearson")

pvals = df_pa %>% select(mean_acti_steps, mean_adept_steps,
         mean_oak_steps_80, mean_scrf_steps,
         mean_scssl_steps, mean_vs_steps_15,
         mean_vs_revised_steps_15, mean_AC, mean_log10AC,
         mean_PAXMTSM, mean_log10PAXMTSM) %>%
  ...
  select(-rowname) %>% 
  as.matrix()
colnames(cor_mat) = rownames(cor_mat) =colnames(pvals) = rownames(pvals) = c("Actilife", "ADEPT","Oak", "Sc. RF", "Sc. SSL", "Vs", "Vs rev", "AC", "log10 AC", "MIMS", "log10 MIMS")
...
#          method="square", 
#          type="upper", 
#          col=paletteer_d("colorBlindness::Green2Magenta16Steps", n = 10), 
#          tl.col="black", 
#          tl.srt=45, 
#          p.mat = pvals,
#          col.lim=c(.2,1),
...
#| eval: false
#| include: false 
ggplot(df_mortality_win,
       aes(x = permth_exm, y = permth_int))+
  geom_point(size = .8) + 
  geom_abline(slope = 1)
# plot(df_mortality_win$permth_exm, df_mortality_win$permth_int)
