...
library(furrr)
force = FALSE
target_files = c(here::here("data", "accelerometry", "summarized", "pa_df_subject_level.rds"),
                 here::here("data", "accelerometry", "summarized", "pa_df_day_level.rds"))

if(!all(file.exists(target_files)) || force){
  if(!dir.exists(here::here("data", "accelerometry", "summarized"))){
    dir.create(here::here("data", "accelerometry", "summarized"), recursive = TRUE)
  }
  ...
  pa_files = list.files(here::here("data", "accelerometry", "minute_level"), pattern = ".*nhanes\\_1440.*csv.xz", recursive = TRUE,
                        full.names = TRUE)
  pa_files = pa_files[!grepl("all", pa_files) & !grepl("PAXFLGSM", pa_files) & !grepl("PAXPREDM", pa_files)]

  pa_vars = sub(".*nhanes\\_1440\\_(.+).csv.xz.*", "\\1", pa_files)

  file = pa_files[1]
  get_day_summaries = function(file){
    # fpath = file.path(here::here("data", "accelerometry", "minute_level"), paste0("nhanes_1440_", variable, ".csv.xz"))
    variable = sub(".*nhanes\\_1440\\_(.+).csv.xz.*", "\\1", file)

    x = read_csv(file, col_types = list(SEQN = col_character())) %>% as.data.table()
    x_filt = x %>%
  ...
  day_summaries = furrr::future_map(.x  = pa_files, .f = get_day_summaries, .progress = TRUE)
  plan(sequential)
